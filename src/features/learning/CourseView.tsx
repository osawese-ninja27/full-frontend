...
import { Page, Container, PageHeader, Badge } from "../../app/ui";
...
export default function CourseView(){
  ...
  return (<Page><Container>
      <PageHeader eyebrow="Student" title="Course" subtitle="Modules unlock in order. You must pass each module's quiz to unlock the next — this is the strict progression gate." />
...
function ModuleBlock({m,index,onChange}:{m:ModuleProgress;index:number;onChange:()=>void}){
  ...
  return (<div className={`rounded-2xl border bg-white p-4 shadow-[var(--shadow-card)] ${m.unlocked?"border-slate-200":"border-slate-200 opacity-70"}`}>
    <div className="flex items-center justify-between">
      ...
      {m.unlocked && <button onClick={toggle} className="text-sm font-medium text-brand-600 hover:underline">{expanded?"Hide":"Open"}</button>}
    </div>
...
function LessonView({lesson}:{lesson:Lesson}){
  ...
  return (<div className="rounded-lg border border-slate-100 bg-slate-50 p-3">
    <div className="flex items-center justify-between">
      ...
      {!monitor.active
        ? <button onClick={monitor.start} className="rounded-lg bg-brand-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-brand-700">Start studying</button>
...
function AssessmentPanel({moduleId,passScore,onPassed}:{moduleId:string;passScore:number;onPassed:()=>void}){
  ...
  if(!a) return null;
  return (<div className="rounded-xl border border-brand-100 bg-brand-50 p-3">
    ...
    <button onClick={submit} className="mt-3 rounded-lg bg-brand-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-brand-700">Submit quiz</button>
